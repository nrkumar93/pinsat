...
def qpos_from_site_pose(physics,
  ...
  mp.mj_fwdPosition(physics.model.ptr, physics.data.ptr)
  ...
  for steps in range(max_steps):
    ...
    if target_quat is not None:
      # Rotational error.
      mp.mju_mat2Quat(site_xquat, site_xmat)
      mp.mju_negQuat(neg_site_xquat, site_xquat)
      mp.mju_mulQuat(err_rot_quat, target_quat, neg_site_xquat)
      mp.mju_quat2Vel(err_rot, err_rot_quat, 1)
    ...
    if err_norm < tol:
      ...
      mp.mj_integratePos(physics.model.ptr, physics.data.qpos, update_nv, 1)
